...
public class LoginController extends ControllerBase {
	...
	public String executeLogin(@ModelAttribute("loginBean") LoginForm loginForm, ModelMap model, BindingResult errors, HttpServletRequest request) {
		if (request.getSession().getAttribute(AppConstants.Session.CURRENT_USER) != null) {
			saveMessage(request.getSession(), "Already logged in.");
			return redirect("/");
		}
		...
		if (isValidLogin) {
			...
			request.getSession().setAttribute(AppConstants.Session.CURRENT_USER, loginForm.getUsername());
		} else {
			errors.reject("errors.login.invalidCombo");
		}
		
		if (errors.hasErrors()) {
			saveError(request, "The username and password combination you provided was not valid.");
			return view("login");
		} else {
